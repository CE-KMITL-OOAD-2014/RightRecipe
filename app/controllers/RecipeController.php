...
class RecipeController extends BaseController
{
	...
	public function postnewrecipe(){
		...
		$newfile=time().".".$file->guessExtension();//new name file
	...
	{		
		$show=Recipe::getById($id);//object recipe
		$showComment=Comment::getByRecipe($id);//all comment of recipeid
		$showCategoey=Category::getById($show->getCategory());//category of object recipe
		$showStar=new RecipeScore;
		$star=$showStar->getStar($id);//average score of recipe
		$username=User::getById($show->getUserId());//username who create recipe
	...
	public function posteditrecipe($id)
	{
		...
		if($file!=NULL){//if change image -> set image
